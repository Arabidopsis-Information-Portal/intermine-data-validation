...
public class UniprotEntrySwissProtItemProcessor implements ItemProcessor<UniprotEntry, UniprotEntry> {

    private static final Logger log = LoggerFactory.getLogger(UniprotEntrySwissProtItemProcessor.class);

    @Override
    public UniprotEntry process(final UniprotEntry item) throws Exception {
    	
    	ValidationStats.uniprotSwissProtInputRecordCount.increment();
        ...
        ValidationStats.uniprotSwissProtProcessedRecordCount.increment();
        
        log.info("Current Statistics:"
				+ ValidationStats.getUniprotSwissProtStatistics());
